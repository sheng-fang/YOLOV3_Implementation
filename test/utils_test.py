...
class MyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.boxes1 = tf.convert_to_tensor([[[1.5, 1.5, 1, 1],
                                            [3.5, 1.5, 1, 1]],
                                            [[1.5, 3.5, 1., 1.],
                                            [3.5, 3.5, 1., 1.]]], dtype=tf.float32)
        self.boxes2 = tf.convert_to_tensor([[[1.5, 1.5, 1, 1],
                                            [4, 1, 1, 1]],
                                            [[1.5, 4.5, 1., 1.],
                                            [3, 4, 1., 1.]]], dtype=tf.float32)

        self.box_ious = tf.convert_to_tensor([[1, 1/7], [0, 1/7]], dtype=tf.float32)

    def test_iou(self):
        self.assertEqual(utils.bbox_iou_tf(self.boxes1, self.boxes2).numpy().tolist(),
    ...
    def test_img_resize(self):
        ...
        img, bbox = utils.img_preprocess_fix_ratio(img_rd, tar_size, box)
